...
func spawn_booey() -> void:
	...
	get_tree().root.add_child(current_bouey)          # add to tree FIRST
	current_bouey.global_position = player.bouey_spawner.global_position  # then set global transform
	current_bouey.set_bobber_stage(player.current_bobber_upgrade)
	...
	current_bouey.current_bouey_upgrade = player.current_bobber_upgrade
